...
namespace Stroids
{
    ...
    {
        private const double RotateSpeed = 200;
        private const double MaxVelocity = 83.3333333333333;
        private const double DecayRate = 0.983333333333333;
        ...
        public void Thrust()
        {
            ...
            VelocityX += -(1.5 * Math.Sin(Radians));
            if (VelocityX > MaxVelocity)
                VelocityX = MaxVelocity;
            if (VelocityX < -MaxVelocity)
                VelocityX = -MaxVelocity;

            VelocityY += 1.5 * Math.Cos(Radians);
            if (VelocityY > MaxVelocity)
                VelocityY = MaxVelocity;
            if (VelocityY < -MaxVelocity)
                VelocityY = -MaxVelocity;

            Sounds.Play(Sound.Thrust);
        }
